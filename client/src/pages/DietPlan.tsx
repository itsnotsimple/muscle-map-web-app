...
const DietPlan = () => {
    ...
    useEffect(() => {
        if (user === null) {
            ...
            let bmr = (10 * Number(savedProfile.weight || 75)) + (6.25 * Number(savedProfile.height || 175)) - (5 * Number(savedProfile.age || 25));
            bmr = (savedProfile.gender || 'male') === 'male' ? bmr + 5 : bmr - 161;
            ...
            const calculatedTdee = Math.round(bmr * (multipliers[savedProfile.activityLevel || 'moderate'] || 1.2));
    ...
    const calculateTDEE = (e?: React.FormEvent) => {
        ...
        setTimeout(() => {
            const { weight, height, age, gender, activityLevel } = form;
            ...
            const calculatedTdee = Math.round(bmr * (multipliers[activityLevel] || 1.2));
            setTdee(calculatedTdee);
